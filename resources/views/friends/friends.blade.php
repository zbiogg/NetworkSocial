...
<div class="zbio-main  container">
  ...
  <div class="col-md-12">
    ...
    @php ($rl1= DB::table('relationship')->where('userID_1',$id)->where('status','1')->count()) @endphp
    @php ($rl2= DB::table('relationship')->where('userID_2',$id)->where('status','1')->count()) @endphp
    <div class="header-friend"><h4><i class="fa fa-users mr-1" aria-hidden="true"></i>Bạn bè ({{$rl1+$rl2}})</h4></div>
    ...
    @foreach(DB::table('relationship')->where('userID_1',$id)->where('status','1')->get() as $r)  
      ...
      <figure class="col-md-3">
        <a href="/profile?id={{$user->id}}" class="black-text">
        <div class="img-profile img-thumbnail" style="background-image: url('@if($user->img_avt=='') img/avt/avt-default.png @else  img/avt/{{$user->img_avt}}  @endif'); min-height:200px;
    background-repeat: no-repeat;
    background-position: center center;
    background-size: cover;"></div>
          <h5 class="text-center my-3">{{$user->lastName}} {{$user->firstName}}</h5>
        </a>
      </figure>
      @endif
    @endforeach
